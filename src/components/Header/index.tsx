              ...
              <div className="w-full md:max-w-[500px] flex-1  flex flex-col relative z-[999]">
                <div className="relative md:translate-x-6">
                  <Input
                    ...
                    onInput={() => setShowSearchBarDropDown(true)}
                    value={filterByNameValue}
                    onChange={changeFilteredValueHandler}
                  />
                  {showSearchBarDropDown && (
                    <SearchBarDropDown className="bg-[#f2f2f2] shadow-lg absolute mt-1 z-[999]" />
                  )}
                </div>
              </div>
              {!token ? (
            ...
            <div className="flex items-center gap-4">
              ...
              {categories.map((category: Icategory) => (
                <Li
                  ...
                  as={`/category/${category.name.split(" ").join("-")}`}
                >
                  <span
                    className={
                      router.query.name === category.name.split(" ").join("-")
